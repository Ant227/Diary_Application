...
class EditProjectFragment : Fragment(){
    ...
    override fun onCreateView(
        ...
        viewModel.project.observe(viewLifecycleOwner, Observer {
            ...
            it.startDate?.let {
               ...
               startMonth  =  SimpleDateFormat("MM").format(it)
               startYear         =  SimpleDateFormat("yyyy").format(it)
                startDate = it
            }
            it.endDate?.let {
                endDay          = SimpleDateFormat("dd").format(it)
                endMonth  =  SimpleDateFormat("MM").format(it)
    ...
    private fun setupDatePicker(){
        val startDateListener = DatePickerDialog.OnDateSetListener { view, year, month, dayOfMonth ->
            ...
            startDate = cal.time
            binding.startDatePicker.text = SimpleDateFormat("MMM,dd yyyy").format(startDate)
